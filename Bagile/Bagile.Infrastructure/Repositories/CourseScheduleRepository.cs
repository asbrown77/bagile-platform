...
public class CourseScheduleRepository : ICourseScheduleRepository
{
    ...
    public async Task UpsertAsync(CourseSchedule schedule)
    {
        const string sql = @"
            ...
            DO UPDATE SET
                name = EXCLUDED.name,
                -- Preserve manual cancellations: once cancelled via platform,
                -- ETL syncs must not overwrite with WooCommerce status.
                status = CASE
    ...
    {
        // Status and Sku are optional — only update them when explicitly provided
        var sql = @"
            UPDATE bagile.course_schedules
            SET name                   = @Name,
